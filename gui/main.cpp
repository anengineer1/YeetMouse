    ...
    if (ImGui::BeginChild("Parameters", ImVec2(220, -1), ImGuiChildFlags_FrameStyle | ImGuiChildFlags_ResizeX)) {
        ...
        switch (selected_mode) {
            ...
            case AccelMode_Power: // Power
            {
                ...
                if (params[selected_mode].useSmoothing) {
#ifdef USE_INPUT_DRAG
                    change |= ImGui::DragFloat("##Motivity_Param", &params[selected_mode].motivity, 0.02, 0.1, 50, "Output Limit %0.2f");
#else
                    change |= ImGui::SliderFloat("##Motivity_Param", &params[selected_mode].motivity, 0.1, 50,
                                                 "Output Limit %0.2f");
#endif
                }
                break;
